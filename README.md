...
│   └── platformio.ini
└── Dashboard/
    ├── app.py                     # Plotly Dash app (entry point)
    ├── data_loader.py
    ├── kinematics.py
    ├── metrics.py
    ├── garmin_compare.py
...
**Location:** Lateral aspect of the thigh, at the midpoint between the greater trochanter (outer hip bony bump) and the lateral femoral condyle (outer knee prominence).
...
The Mahony filter calibrates by measuring gravity on the IMU Z-axis, requiring all sensors to be approximately horizontal during calibration.
...
4. Lay sensors flat and wait for calibration to complete (~30 s)
...
2. Click **Load Run** and allow ~3 seconds for the skeleton animation to compute
3. All three panels populate automatically

### Panel 1: 3D Skeleton Animation

Blue = left leg, orange = right leg. Hip at top (Y = 0), toes at bottom. Use the embedded **Play** button and scrub bar to control playback (0.5x real-time). Rotate and zoom with click-and-drag. Y axis = vertical, Z axis = sagittal (forward/back swing).

### Panel 2: Joint Angles

Knee flexion and ankle dorsiflexion over time, left and right overlaid. 0 degrees knee = full extension, 60+ degrees = peak swing flexion. Positive ankle = dorsiflexion, negative = plantarflexion. Hover for exact values.
...
| Symmetry index | `|L - R| / ((L + R) / 2) x 100` for knee, ankle, cadence | <5% normal |
| Fatigue comparison | Mean knee flexion angle, first 10% vs last 10% of run | Delta >2 deg flagged |
...
**"Knee angle stuck near 0 degrees"**
